...
void timer(int var)
{
    ...
    while(read(fd, &buf, 1)) //читаем файл посимвольно, ищем символ переноса строки
    {
        printf("%c", buf);
    }
    close(fd);
...
int main()
{
    ...
    int lenght[200]; //А тут длину строк по номерам
    ...
    while(read(fd, &buf, 1)) //читаем файл посимвольно, ищем символ переноса строки
    {
        ...
        {
            len++;
            int test = lseek(fd, 0L, 1); //смотрим, сколько бит до начала файла
